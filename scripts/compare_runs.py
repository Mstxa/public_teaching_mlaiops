...
def to_markdown_table(df) -> str:
    """Render without pandas' to_markdown, which needs `tabulate` — a package this
    repository does not pin. `make compare` crashed on a clean install because of it."""
    cols = [str(c) for c in df.columns]
    rows = [[("" if v is None else str(v)) for v in rec] for rec in df.itertuples(index=False)]
    width = [max(len(cols[i]), *(len(r[i]) for r in rows)) if rows else len(cols[i])
             for i in range(len(cols))]
    out = ["| " + " | ".join(c.ljust(width[i]) for i, c in enumerate(cols)) + " |",
           "|" + "|".join("-" * (width[i] + 2) for i in range(len(cols))) + "|"]
